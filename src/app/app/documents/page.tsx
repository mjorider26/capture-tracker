...
export default async function DocumentsPage() { const context = await getContext(); const documents = await listDocuments(context.business.id); return <AppShell mode="app" destination="documents" businessName={context.business.displayName}><div className="space-y-6"><DocumentUploadForm /><DocumentsExperience documents={documents} basePath="/app" /></div></AppShell>; }
