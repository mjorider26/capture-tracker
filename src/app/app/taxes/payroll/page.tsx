...
import { PayrollReversalExperience } from "@/components/payroll-reversal-experience";
...
import { matchAuthenticatedPayrollEvidence, recordAuthenticatedPayroll, reverseAuthenticatedPayroll } from "./actions";
...
export default async function PayrollPage() {
  ...
  const [data, accounts, documents, runs, externalTransactions] = await Promise.all([
    ...
    prisma.payrollRun.findMany({ where: { businessId: context.business.id, status: "PROCESSED" }, select: { id: true, payDate: true, grossWages: true }, orderBy: { payDate: "desc" }, take: 50 }),
  ...
  return (
    ...
    >
      ...
      <PayrollReversalExperience action={reverseAuthenticatedPayroll} runs={runs.map((run) => ({ id: run.id, payDate: run.payDate.toISOString().slice(0, 10), grossWages: run.grossWages.toFixed(2) }))} />
    </AppShell>
  );
}
