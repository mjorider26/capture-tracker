...
export async function classifyAuthenticatedOwnerTransfer(_previous: OwnerMoneyActionState, formData: FormData): Promise<OwnerMoneyActionState> { try { const context = await requireBusinessContext(); const result = await classifyOwnerTransfer(prisma, { businessId: context.business.id, actorUserId: context.user.id, actorMembershipId: context.membership.id, role: context.membership.role, executionMode: "authenticated" }, Object.fromEntries(formData)); if (!result.ok) return { status: "error", message: result.message }; revalidatePath("/app/taxes/owner-money"); revalidatePath("/app/today"); revalidatePath("/app/review"); return { status: "success", message: "Owner transfer review recorded without posting accounting." }; } catch { return { status: "error", message: "The transfer review could not be saved safely." }; } }
