...
export default async function DemoReconciliations() {
  const context = await resolveLocalDemoContext();
  if (!context) notFound();
  const records = await getReconciliations(context.businessId);
  return (
    <AppShell mode="demo" destination="money" navigationDestination="reconciliation" businessName={context.businessName}>
      ...
      {records.map((record) => (
        <Link key={record.id} href={`/demo/money/reconciliations/${record.id}`} className="ui-card mb-3 block p-5">
