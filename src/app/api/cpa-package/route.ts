...
export async function GET(request: NextRequest) {
  try {
    ...
    const files: ExportFile[] = [
      ...
      { name: "fixed-assets.csv", content: csv(["Asset", "Category", "Vendor", "Acquisition date", "Cost", "Placed in service", "Status", "CPA notes"], assets.map((item) => [item.name, item.category, item.vendor, date(item.acquisitionDate), money(item.acquisitionCost), date(item.placedInServiceDate), item.status, item.cpaNotes])) },
