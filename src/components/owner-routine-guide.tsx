...
export function OwnerRoutineGuide({ basePath }: { basePath: "/app" | "/demo" }) {
  ...
  return <section className="space-y-6">
    ...
    <section className="ui-card p-5 sm:p-6"><h2 className="text-lg font-bold text-brand-navy">Keep bank activity current</h2><p className="mt-2 text-sm leading-6 text-text-muted">You can either <strong className="text-text-primary">connect automatically</strong>, so Capture Tracker syncs activity through your connected financial institution, or <strong className="text-text-primary">import manually</strong> by uploading transaction CSV files from your bank or card provider. Either way, transactions enter the same review process before they become accounting.</p><div className="mt-4 flex flex-wrap gap-4 text-sm font-bold"><Link className="ui-link" href={`${basePath}/money/bank`}>Bank activity settings →</Link><Link className="ui-link" href={`${basePath}/money/import`}>Import transactions →</Link></div></section>
