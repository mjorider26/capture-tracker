...
export function AppPullToRefresh({ children }: { children: React.ReactNode }) {
  const router = useRouter(); const start = useRef<{ x: number; y: number } | null>(null); const tracking = useRef(false); const refreshing = useRef(false); const phaseRef = useRef<Phase>("idle");
  ...
  const reset = () => { start.current = null; tracking.current = false; setDistance(0); setCurrentPhase("idle"); };
  ...
  const onMove = (event: TouchEvent<HTMLDivElement>) => { const touch = event.touches[0]; if (!touch || !tracking.current || !start.current) return; const deltaX = touch.clientX - start.current.x; const deltaY = touch.clientY - start.current.y; if (Math.abs(deltaX) > Math.abs(deltaY) || deltaY <= 0) { reset(); return; } const next = resistedPullDistance(deltaY); setDistance(next); setCurrentPhase(pullState(deltaY)); if (next > 0) event.preventDefault(); };
  const refresh = async () => { if (refreshing.current) return; refreshing.current = true; setCurrentPhase("refreshing"); try { router.refresh(); await wait(350); setCurrentPhase("complete"); await wait(350); reset(); } catch { setCurrentPhase("error"); await wait(1600); reset(); } finally { refreshing.current = false; } };
  const onEnd = () => { const armed = phaseRef.current === "armed"; start.current = null; tracking.current = false; setDistance(0); if (armed) void refresh(); else if (phaseRef.current !== "refreshing") setCurrentPhase("idle"); };
