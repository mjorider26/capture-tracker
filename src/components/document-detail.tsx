...
export function DocumentDetail({ document, basePath, actions, contentHref }: { document: Detail; basePath: "/app" | "/demo"; actions?: ReactNode; contentHref?: string }) {
  ...
  return <section className="space-y-6">
    ...
    <Card className="p-5 sm:p-6">
      ...
      {contentHref ? <a href={contentHref} className="mt-6 inline-flex min-h-11 items-center rounded-[var(--radius-sm)] bg-brand-teal px-4 text-sm font-bold text-white shadow-sm transition-colors hover:bg-[var(--brand-teal-strong)]">Open protected document</a> : <p className="mt-6 rounded-[var(--radius-sm)] bg-surface-secondary p-4 text-sm leading-6 text-text-muted">Preview and download are unavailable until an active document is available in private storage.</p>}
      {actions}
    </Card>
    <Card className="p-5 sm:p-6"><h2 className="text-lg font-bold">Status history</h2><ol className="mt-5 space-y-4">{document.statusHistory.map((event) => <li key={event.id} className="border-l-2 border-brand-teal pl-4 text-sm"><p className="font-semibold text-text-primary">{display(event.newStatus)}</p><p className="mt-1 text-text-muted">{date(event.createdAt)}{event.reasonCode ? ` / ${display(event.reasonCode)}` : ""}</p></li>)}</ol></Card>
  </section>;
}
