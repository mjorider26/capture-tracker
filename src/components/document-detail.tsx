  ...
  return <section className="space-y-6">
    ...
    <Card className="document-safeguards p-5 sm:p-6">
      ...
      <dl className="mt-5 grid gap-5 text-sm sm:grid-cols-2">
        ...
        <div><dt className="text-text-muted">Security scan</dt><dd className="mt-1 font-semibold text-text-primary">{scanMessage}</dd></div>
      ...
      {contentHref ? <a href={contentHref} className="ui-button ui-button-primary mt-6 inline-flex min-h-11 items-center rounded-[var(--radius-sm)] bg-brand-teal px-4 text-sm font-bold text-white shadow-sm transition-colors hover:bg-[var(--brand-teal-strong)]">Open protected document</a> : <p className="auth-note mt-6 rounded-[var(--radius-sm)] p-4 text-sm leading-6 text-text-muted">Preview and download are unavailable until the private document passes its security scan.</p>}
      {actions}
    </Card>
