...
export function TransactionDetailExperience({
  ...
  return (
    <>
      ...
      <Card className="mt-5 p-5"><h2 className="font-bold">Explain this transaction</h2><p className="mt-1 text-sm text-text-muted">A deterministic evidence chain — no AI inference.</p><div className="mt-4 space-y-3 text-sm"><Explain label="Source" value={`${detail.explanation.source.account} · ${detail.explanation.source.imported ? "Imported bank/card activity" : "Recorded transaction"} · ${detail.explanation.source.description} · $${detail.explanation.source.amount}`} /><Explain label="Document" value={detail.explanation.documents.length ? detail.explanation.documents.map((item) => item.name).join(", ") : "No trusted receipt/document is linked."} /><Explain label="Classification" value={[detail.explanation.classification.category, detail.explanation.classification.suggestionReason, detail.explanation.classification.merchantRule ? "Merchant-rule evidence available" : null, detail.explanation.classification.policyReference].filter(Boolean).join(" · ") || "No classification evidence is available."} /><Explain label="Approval" value={detail.explanation.approval.decidedAt ? `${detail.explanation.approval.status} · ${new Date(detail.explanation.approval.decidedAt).toLocaleString()}` : `${detail.explanation.approval.status} · no approval timestamp recorded`} /><Explain label="Accounting" value={detail.explanation.accounting.entryNumber ? `${detail.explanation.accounting.entryNumber} · posted ${detail.explanation.accounting.postingDate ? new Date(detail.explanation.accounting.postingDate).toLocaleDateString() : "date unavailable"} · debits $${detail.explanation.accounting.debits} · credits $${detail.explanation.accounting.credits}` : "No journal entry is recorded."} /><Explain label="Changes" value={detail.correction.reversal || detail.journal?.reversal ? "A reversal or correction is linked below; original history is retained." : "No reversal or correction is linked."} /><Explain label="Audit" value={detail.explanation.audit.length ? detail.explanation.audit.map((item) => `${item.action} ${new Date(item.occurredAt).toLocaleDateString()}`).join(" · ") : "No lifecycle audit events are available."} /></div></Card>
...
function Explain({ label, value }: { label: string; value: string }) { return <div className="border-t border-border-subtle pt-3"><p className="font-bold">{label}</p><p className="mt-1 text-text-muted">{value}</p></div>; }
