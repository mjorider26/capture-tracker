...
describe("FinancialReports operational navigation", () => {
  ...
  it("does not advertise owner-only money workflows to review roles", () => {
    const html = renderToStaticMarkup(
      <FinancialReports
        basePath="/app"
        focus="cash-activity"
    ...
    expect(html).not.toContain('href="/app/taxes/owner-money"');
    expect(html).toContain('href="/app/reports/operations?report=mileage-log"');
  });
});
