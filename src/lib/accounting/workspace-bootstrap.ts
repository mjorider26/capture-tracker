...
/**
 * Deterministic foundation writes. Keeping these as one operation list lets
 * invitation acceptance include the complete accounting foundation in its
 * provisioning transaction instead of creating a partially usable tenant.
 */
export function workspaceAccountingFoundationOperations(client: Database, businessId: string, now = new Date()) {
  ...
  return [
    client.financialAccount.upsert({
      where: { id: checkingId },
...
export async function ensureWorkspaceAccountingFoundation(businessId: string, now = new Date()) {
  await prisma.$transaction(workspaceAccountingFoundationOperations(prisma, businessId, now));
}
