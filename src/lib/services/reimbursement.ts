...
export async function createPersonallyPaidReimbursement(client: Client, actor: ReimbursementActor, input: unknown): Promise<CreateReimbursementResult> {
  ...
  try {
    return await client.$transaction(async (tx) => {
      ...
      const policyVersion = await tx.accountingPolicyVersion.findFirst({ where: { businessId: actor.businessId, effectiveDate: { lte: new Date(`${data.incurredAt}T23:59:59.999Z`) }, policy: { policyType: "ACCOUNTABLE_PLAN" } }, orderBy: { effectiveDate: "desc" }, select: { id: true } });
      if (policyVersion) await tx.accountingPolicyApplication.create({ data: { businessId: actor.businessId, policyVersionId: policyVersion.id, entityType: "ReimbursementClaim", entityId: claim.id } });
      await tx.auditEvent.create({ data: { actorType: "USER", businessId: actor.businessId, actorMembershipId: actor.actorUserId, action: "CREATE", entityType: "ReimbursementClaim", entityId: claim.id, afterJson: { status: "DRAFT", amount: data.amount, personallyPaid: true, hasSupportingDocument: Boolean(data.documentId), policyVersionId: policyVersion?.id ?? null }, metadataJson: { executionMode: actor.executionMode, reimbursementSubmission: true } } });
