...
export const taxPaymentSchema = z.object({ estimateId: id, expectedVersion: version, amount: money, paidAt: date, notes: z.string().trim().max(240).refine((value) => !/[\x00-\x1F\x7F]/.test(value), "Notes contain unsupported characters.").optional().transform((value) => value || null), idempotencyKey: key });
