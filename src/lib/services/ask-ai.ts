...
export function localAskAiAdapter(question: string, context: Context): AdapterResult {
  ...
  if (/week|review|transaction/.test(q)) { const transactions = context.review.tasks.filter((task) => task.category === "Transactions").length; return { answer: `Your Weekly Review is ${context.review.review?.status?.toLowerCase() ?? "not started"}. It currently identifies ${context.review.tasks.length} unresolved record tasks, including ${transactions} transaction tasks.`, aliases: ["review"], followUps: ["What documents need attention?"] }; }
