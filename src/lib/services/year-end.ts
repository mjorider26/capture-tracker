...
export async function getYearEndReadiness(client: PrismaClient, businessId: string, year: number) {
  ...
  const [imports, transfers, reimbursements, payroll, assets, documents, reconciliations, closedMonths] = await Promise.all([
  ...
  return { year, status: issues.length ? "ISSUES_REMAIN" as const : "READY_FOR_CPA" as const, checks, issues };
}
