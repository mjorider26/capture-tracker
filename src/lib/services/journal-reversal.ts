...
export async function reverseJournalEntry(client: Client, actor: ReconciliationActor, input: unknown): Promise<JournalReversalResult> {
  ...
  try { return await client.$transaction(async (tx) => {
    ...
    if (original.transactionId) await tx.transaction.updateMany({ where: { id: original.transactionId, businessId: actor.businessId, status: { not: "VOIDED" } }, data: { status: "VOIDED", voidedAt: new Date(), version: { increment: 1 } } });
