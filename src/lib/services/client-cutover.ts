...
export async function saveClientCutover(actor: Actor, form: FormData): Promise<CutoverState> {
  const ownerDisplayName = clean(form.get("ownerDisplayName"), 120); const displayName = clean(form.get("displayName"), 160); const legalName = clean(form.get("legalName"), 160); const timezone = clean(form.get("timezone"), 64); const fiscal = Number(form.get("fiscalYearStartMonth")); const cutoverDate = date(clean(form.get("cutoverDate"), 10)); const sourceReference = clean(form.get("sourceReference"), 300); const accountName = clean(form.get("accountName"), 120); const institutionName = clean(form.get("institutionName"), 120) || null; const lastFour = clean(form.get("lastFour"), 4) || null; const accountType = clean(form.get("accountType"), 24); const openingBalance = amount(clean(form.get("openingBalance"), 19));
  if (!ownerDisplayName || !displayName || !legalName || !cutoverDate || !sourceReference || !accountName || !openingBalance || !["CHECKING", "CREDIT_CARD"].includes(accountType) || !/^America\/[A-Za-z_]+$/.test(timezone) || !Number.isInteger(fiscal) || fiscal < 1 || fiscal > 12 || form.get("ownerConfirmed") !== "on") return { ok: false, message: "Enter approved source facts, a valid cutover date, and owner confirmation." };
  if (openingBalance.equals(0)) return { ok: false, message: "A zero balance does not create an opening journal. Use a supported account setup with an approved non-zero source balance." };
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (!onboarding || !cutover || cutover.openingJournalId) throw new Error("CUTOVER_ALREADY_RECORDED");
      ...
      await tx.financialAccount.update({ where: { id: account.id }, data: { name: accountName, institutionName, lastFour, type: accountType as "CHECKING" | "CREDIT_CARD", ownership: "BUSINESS", openingBalance, openedAt: cutoverDate } });
      ...
      await tx.businessCutover.update({ where: { businessId: actor.businessId }, data: { startDate: cutoverDate, sourceReference, openingJournalId: entry.id, version: { increment: 1 } } });
      await tx.businessOnboarding.update({ where: { businessId: actor.businessId }, data: { actorUserId: actor.actorUserId, ownerDisplayName, cutoverDate, openingBalancesPosted: true, ownerMoneyInitialized, payrollYtdEstablished, fixedAssetsReviewed, status: "IN_PROGRESS", completedAt: null } });
      await tx.auditEvent.create({ data: { actorType: "USER", businessId: actor.businessId, actorMembershipId: actor.actorUserId, action: "CREATE", entityType: "OpeningBalanceCutover", entityId: entry.id, afterJson: { cutoverDate: cutoverDate.toISOString().slice(0, 10), balanced: true, accountType, openingBalance: positive.toFixed(2) }, metadataJson: { sourceReference, ownerConfirmed: true, quickBooksApiUsed: false } } });
...
export async function markInitialReconciliationComplete(businessId: string) {
  ...
  await prisma.businessOnboarding.update({ where: { businessId }, data: { initialReconciliationComplete: true, status: complete ? "COMPLETED" : "IN_PROGRESS", completedAt: complete ? new Date() : null, booksCurrentThrough: complete ? new Date() : null } });
}
