...
export async function getActivity(businessId: string, raw: ActivityQuery = {}) {
  ...
  const [audits, docs, reviews, ai, settings, exports] = await Promise.all([
    prisma.auditEvent.findMany({ where: { businessId, entityType: { notIn: ["DocumentScanTrace", "DocumentRemovalTrace"] } }, orderBy: { occurredAt: "desc" }, take: 100, select: { id: true, entityType: true, entityId: true, action: true, occurredAt: true, metadataJson: true, afterJson: true } }),
