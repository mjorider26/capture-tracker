...
export async function reverseJournalFromForm(actor: ReconciliationActor, formData: FormData): Promise<AccountingActionState> { const result = await reverseJournalEntry(prisma, actor, { journalEntryId: formData.get("journalEntryId"), expectedVersion: formData.get("expectedVersion"), reversalDate: formData.get("reversalDate"), reason: formData.get("reason") }); return result.ok ? { status: "success", message: "Posted reversal created; the original entry remains unchanged." } : { status: result.code === "CONFLICT" ? "conflict" : "error", message: result.message }; }
