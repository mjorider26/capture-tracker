...
describe("operator invitation acceptance", () => {
  ...
  it("rejects a wrong email, expired invitation, and concurrent reuse before creating a business", async () => {
    ...
    const client = { $transaction: vi.fn(async (callback: (value: typeof tx) => Promise<unknown>) => callback(tx)) }; const { acceptOperatorInvitation, InvitationError } = await import("./operator-invitations");
    await expect(acceptOperatorInvitation({ token, userId: "attacker", email: "attacker@example.test", client: client as never })).rejects.toMatchObject({ code: "FORBIDDEN" } satisfies Partial<InvitationError>); expect(tx.business.create).not.toHaveBeenCalled();
