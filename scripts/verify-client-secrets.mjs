...
const forbidden = ["DATABASE_URL", "DIRECT_DATABASE_URL", "CLOUDFLARE_API_TOKEN", "postgresql://", "-----BEGIN PRIVATE KEY-----"];
...
const secretValues = existsSync(envFile)
  ? readFileSync(envFile, "utf8").split(/\r?\n/).flatMap((line) => {
      const match = line.match(/^\s*(BETTER_AUTH_SECRET|DATABASE_URL|DIRECT_DATABASE_URL|CLOUDFLARE_API_TOKEN)=(.*)$/);
      if (!match) return [];
