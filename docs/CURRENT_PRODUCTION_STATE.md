...
- Plaid is not activated in production. The Plaid/CSV customer-choice implementation is a source release candidate only until exact-SHA CI, private Trial credentials, exact-SHA owner release authorization, migration, and supported production smoke checks complete. The current production release remains unchanged, and CSV import remains the complete supported accounting-ingestion path.
