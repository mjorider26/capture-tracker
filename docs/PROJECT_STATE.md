...
- Phase 9B repository preparation adds fictional-staging configuration guards, migration/bootstrap safeguards, future smoke and Neon verification tools, Linux CI preparation, bundle verification tooling, and boundary/recovery/cost documentation.

## Current boundary

Local/test data and free-preview staging are fictional-only. Production is undeployed, `CAPTURE_TRACKER_REAL_DATA_APPROVED=false`, and no real customer onboarding is allowed. No Cloudflare, Neon, R2, or AWS resource currently exists. R2 remains deferred. No restore drill or live cloud cost verification has occurred.

## Recommended next work

**Phase 9B External Session â€” Create Free Neon and Cloudflare Staging Resources, Deploy Fictional Preview, and Validate Runtime.**

Pending external-session work: Linux OpenNext build, actual bundle measurement, provider account setup, Neon migrations, fictional cloud bootstrap, Worker deployment, reachable browser testing, Neon integrity and concurrency proof, backup/restore drill, and live cost verification.
