...
- Require the uploaded Linux `.artifacts/runtime-audit-report.json` from `cloud:audit:report`: its endpoint must be available, its lockfile-clean flag true, its sanitizer flag false, and it must contain no unresolved runtime or high/critical request-time Worker advisory. Do not use a local unavailable audit result as release evidence.
