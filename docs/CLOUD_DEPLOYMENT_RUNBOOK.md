...
- Confirm production is undeployed, real-data approval is false, R2 is deferred, and no AWS resource is in scope.
...
3. Run `npm run cloud:config:verify`, `npm run cloud:phase-9b:verify`, and the Linux CI workflow. Linux OpenNext build and actual Worker-bundle measurement remain pending until that CI run occurs.
